...
import { createFilingsResponse, parseQueryParams, createQueryString } from './filings.service';
...
export async function filingsController(req, res) {
  ...
  const response = await fetch(`${EDGAR_BASE_URL}${createQueryString(tickerSymbol, page, count)}`);
  ...
  const filingsResponse = Object.assign(createFilingsResponse($, filingTypeTds), { page, count });
